...
/**
 * Service for interacting with Deezer API using axios with urlProxy for CORS handling
 */
class DeezerService {
	...
	async getTrack(trackId: string): Promise<DeezerTrack> {
		try {
			...
			// Use axios with urlProxy for CORS handling
			const response = await axios.get(urlProxy(trackUrl), {
				timeout: 10000,
				validateStatus: (status) => status >= 200 && status < 400,
			});

			// Validate response data
			const data: DeezerApiTrack = response.data;
	...
	private async getTrackFromShortLink(shortCode: string): Promise<DeezerTrack> {
		try {
			const shortUrl = urlProxy(`https://link.deezer.com/s/${shortCode}`);

			// Try to resolve the shortened link using axios
			// First attempt: follow redirects to get the final URL
			const response = await axios.get(urlProxy(shortUrl), {
				maxRedirects: 5,
				validateStatus: (status) => status >= 200 && status < 400,
				timeout: 10000,
			});
	...
	/**
	 * Search for tracks on Deezer using axios with urlProxy for CORS handling
	...
	async searchTrack(query: string, limit = 10): Promise<DeezerTrack[]> {
		try {
			...
			// Use axios with urlProxy for CORS handling
			const response = await axios.get(urlProxy(searchUrl), {
			...
			// Validate response data
			const data: DeezerApiResponse = response.data;
	...
	async findTrackMatches(sourceTrack: {
		...
		const searchQueries = [
			// Most specific: artist + title
			`${artists[0]} ${name}`,
			// Just title if artist search fails
			name,
		];
